...
function Products({ addToCart }) {
  ...
  // Helper to calculate price based on weight multiplier
  const getProductPrice = (basePrice, weight) => {
    if (weight === "500g") return (basePrice * 1.9).toFixed(2);
    if (weight === "1kg" || weight === "1kg Set") return (basePrice * 3.6).toFixed(2);
    if (weight === "2kg Set") return (basePrice * 7.0).toFixed(2);
    return basePrice.toFixed(2);
  };

  // Helper to calculate original price for strike-through
  const getProductOriginalPrice = (baseOriginalPrice, weight) => {
    if (weight === "500g") return (baseOriginalPrice * 1.9).toFixed(2);
    if (weight === "1kg" || weight === "1kg Set") return (baseOriginalPrice * 3.6).toFixed(2);
    if (weight === "2kg Set") return (baseOriginalPrice * 7.0).toFixed(2);
  ...
  return (
    <main className="nutridry-products-page">
      ...
      <section className="products-hero-banner">
        <div className="container text-center">
          <span className="products-banner-tag animate-fade-in">
            <i className="bi bi-patch-check-fill me-1"></i> 100% Farm Fresh Guarantee
          </span>
          ...
          <p className="products-banner-desc animate-fade-up">
            Explore our artisanal collection of handpicked almonds, creamy cashews, crisp pistachios, rich walnuts, and organic superfood mixes.
          </p>
          ...
          <nav className="products-breadcrumb d-flex justify-content-center align-items-center gap-2">
            ...
            {selectedCategory !== "all" && (
              <>
                <i className="bi bi-chevron-right"></i>
                <span className="active-cat-crumb">{selectedCategory}</span>
              </>
            )}
          </nav>
        </div>
      ...
      {/* =========================================================
          MAIN PRODUCTS CATALOG & FILTER CONTROLS
          ========================================================= */}
            ...
            <div className="row g-3 align-items-center">
              ...
              <div className="col-6 col-md-3 col-lg-4 d-flex justify-content-end">
                <div className="sort-dropdown-box d-flex align-items-center gap-2">
                  <label htmlFor="sortSelect" className="sort-label d-none d-sm-inline">
                    Sort:
                  </label>
                  <select
                    id="sortSelect"
          ...
          {filteredProducts.length > 0 ? (
            <div className="row g-4">
              {filteredProducts.map((product) => {
                ...
                const activePrice = getProductPrice(product.price, currentWeight);
                const activeOriginalPrice = getProductOriginalPrice(product.originalPrice, currentWeight);
                ...
                return (
                  <div key={product.id} className="col-12 col-sm-6 col-lg-4 col-xl-3">
                    <div className="nutri-product-card h-100">
                      ...
                      {/* Product Image Container with Zoom Hover Effect */}
                      ...
                      <div className="product-card-info p-3 d-flex flex-column flex-1">
                        ...
                        <h3 className="product-title-heading">{product.name}</h3>
                        ...
                        <div className="weight-selection-box mb-3">
                          <span className="weight-label">Select Pack:</span>
                          <div className="weight-options d-flex gap-1 mt-1">
                            {product.weightOptions.map((weight) => (
                              <button
                                key={weight}
                                type="button"
                                className={`btn-weight-pill ${currentWeight === weight ? "active" : ""}`}
                        ...
                        <div className="benefits-tags-box d-flex flex-wrap gap-1 mb-3">
                          {product.benefits.map((b, idx) => (
                            <span key={idx} className="health-tag">
                              <i className="bi bi-check2 text-success me-1"></i> {b}
                            </span>
                          ))}
                        </div>
                        ...
                        <div className="card-action-footer d-flex align-items-center justify-content-between pt-3 border-top mt-auto">
                          <div className="price-stack">
                            <div className="d-flex align-items-baseline gap-2">
                              <span className="active-price">${activePrice}</span>
                              <span className="active-old-price">${activeOriginalPrice}</span>
                            </div>
                            <span className="price-weight-sub">Pack of {currentWeight}</span>
                          </div>
            ...
            <div className="no-products-found-card text-center py-5">
              ...
              <h3 className="empty-title">No Dry Fruits Found</h3>
              <p className="empty-subtitle text-muted">
                We couldn't find any products matching "<strong>{searchQuery}</strong>" in <strong>{selectedCategory}</strong>.
              </p>
              <button
                type="button"
